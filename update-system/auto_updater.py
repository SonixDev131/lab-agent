...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("update.log")],
)
...
def check_and_update():
    ...
    try:
        logger.info(
            f"Bắt đầu quá trình kiểm tra cập nhật. Phiên bản hiện tại: {CURRENT_VERSION}"
        )
        ...
        if result:
            ...
            logger.warning(
                "Không có cập nhật mới hoặc quá trình cập nhật không thành công."
            )
            return False
