...
def main():
    ...
    try:
        ...
        if update_result:
            logger.info("Đã khởi tạo quá trình cập nhật, ứng dụng sẽ khởi động lại sau khi hoàn tất.")
            # Quá trình cập nhật sẽ tự động khởi động lại ứng dụng sau khi hoàn tất
            return
        logger.info("Không có cập nhật mới hoặc cập nhật không thành công. Tiếp tục khởi động ứng dụng.")
    ...
    listener_thread = threading.Thread(target=listener.start, name="CommandListenerThread")
